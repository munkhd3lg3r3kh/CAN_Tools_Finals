...
def Replay_attack(data_iloc):
    ...
    hex_data = hex(Fuzzing_attack.ID)[2:].upper()
    for _ in range(4 - len(hex_data)):
        hex_data = '0' + hex_data
    
    all_datas += hex_data + "\t" + str(Fuzzing_attack.MSGTYPE) + "\t" + str(Fuzzing_attack.LEN)
    ...
    for i in range(int(data_iloc[4])):
        ...
        data_hex = hex(Fuzzing_attack.DATA[i])[2:].upper()  
